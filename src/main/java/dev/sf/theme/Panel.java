...
public class Panel extends PanelBase<IPanelItem> {
        ...
        if (open) {
            ...
            if (height > 0) {
                for (ModuleItem frame : moduleItems) {
                    if (NhackPlugin.guiHandler != null && !NhackPlugin.guiHandler.matchesSearch(frame)) continue;
                    frame.setX(x);
                    frame.setY(y0);
    ...
    public double getHeight() {
        ...
        for (ModuleItem item : moduleItems) {
            if (NhackPlugin.guiHandler != null && !NhackPlugin.guiHandler.matchesSearch(item)) continue;
    ...
    public boolean charTyped(char character) {
        if (open) moduleItems.forEach(frame -> { if (NhackPlugin.guiHandler == null || NhackPlugin.guiHandler.matchesSearch(frame)) frame.charTyped(character); });
        return false;
    }

    @Override
    public boolean keyTyped(int key, int scanCode, int modifiers) {
        if (open) moduleItems.forEach(frame -> { if (NhackPlugin.guiHandler == null || NhackPlugin.guiHandler.matchesSearch(frame)) frame.keyTyped(key, scanCode, modifiers); });
        return false;
    }
