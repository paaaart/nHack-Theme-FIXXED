...
public class ClickGUIHandler extends PanelHandlerBase<Panel> implements EventListener {
    ...
    /**
     * If the user drags the Theme > GuiScale slider, applying the new scale immediately can
     * cause visual flicker/jitter because mouse coordinates are transformed by the scale while
     * the slider value itself is also based on those same coordinates.
     *
     * We freeze the global scale used for rendering/input while the slider is being dragged,
     * then apply the new value on mouse release.
     */
    private boolean freezeGlobalScale = false;
    private float frozenGlobalScale = 1.0f;
...
private static float clampScale(float v) {
    ...
    public float getScale() {
        // If we're currently dragging the GuiScale slider, keep scale stable to avoid jitter.
        if (freezeGlobalScale) return frozenGlobalScale;
        return getScaleUnfrozen();
    }

    private float getScaleUnfrozen() {
    ...
    /** Freeze global GuiScale until {@link #endGuiScaleDrag()} is called. */
    public void beginGuiScaleDrag() {
        if (!freezeGlobalScale) {
            // Freeze at the current (unfrozen) value so render/input stays consistent during drag.
            frozenGlobalScale = getScaleUnfrozen();
            freezeGlobalScale = true;
        }
    }

    /** Unfreeze global GuiScale so the newly-set value applies immediately. */
    public void endGuiScaleDrag() {
        freezeGlobalScale = false;
    }
