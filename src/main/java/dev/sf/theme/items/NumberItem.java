...
public class NumberItem extends ExtendableItem {
    ...
    public void mouseReleased(double mouseX, double mouseY, int button) {
        ...
        if (button == GLFW_MOUSE_BUTTON_LEFT) {
            // Stop drag-updating numeric settings.
            isListening = false;

            // Special-case: if we were dragging the global GuiScale setting, unfreeze scale now
            // so the new value applies once the user releases the mouse button.
            if (setting == NhackPlugin.theme.guiScale && NhackPlugin.guiHandler != null) {
                NhackPlugin.guiHandler.endGuiScaleDrag();
            }
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
                ...
                : panel.isHovering(mouseX, mouseY, getX(), getY(), getWidth() - 1 - 14, getHeight(false)))
        {
            if(button == org.lwjgl.glfw.GLFW.GLFW_MOUSE_BUTTON_1) {
                ...
                // Special-case: avoid flicker when dragging Theme > GuiScale by freezing the
                // global scale used for rendering/input during the drag.
                if (setting == NhackPlugin.theme.guiScale && NhackPlugin.guiHandler != null) {
                    NhackPlugin.guiHandler.beginGuiScaleDrag();
                }
            }
